...
struct MessageSendFailedError: CIErrorProtocol {
    ...
    init(error: NSError, message: Message) {
        self.error = error
...
enum CIError: Error, CIErrorProtocol {
    ...
    var nsError: NSError {
        return NSError(domain: "com.Sparks.chat", code: code, userInfo: userInfo)
    }
}
...
final class HttpAPIImpl : NSObject, HttpAPI {
    ...
    private func configureDefaultSessionManager() {
        ...
        self.sessionManager = Alamofire.SessionManager(configuration: defaultConfiguration)
    }

    private func configureBackgroundSessionManager() {
        let configuration = URLSessionConfiguration.background(withIdentifier: "com.Sparks.chat.backgroundtransfer")
        configuration.shouldUseExtendedBackgroundIdleMode = true
        configuration.sessionSendsLaunchEvents = true
        configuration.isDiscretionary = true
        configuration.timeoutIntervalForResource = HttpConsts.uploadTimeout
        configuration.requestCachePolicy = .reloadIgnoringLocalAndRemoteCacheData
        configuration.httpMaximumConnectionsPerHost = HttpConsts.maxHttpRequests
